...
static inline unsigned long int taus113_get(taus113_state_t *state);
...
typedef uint8_t *(*MedianFuncString)(size_t n,
  const size_t *lengths,
  const uint8_t *strings[],
  const double *weights,
  size_t *medlength);
typedef wchar_t *(*MedianFuncUnicode)(size_t n,
  const size_t *lengths,
...
typedef uint8_t *(*MedianImproveFuncString)(size_t len, const uint8_t *s,
  size_t n,
  const size_t *lengths,
  const uint8_t *strings[],
  const double *weights,
  size_t *medlength);
typedef wchar_t *(*MedianImproveFuncUnicode)(size_t len, const wchar_t *s,
  size_t n,
  const size_t *lengths,
  const wchar_t *strings[],
  const double *weights,
...
typedef double(*SetSeqFuncString)(size_t n1,
  ...
  const uint8_t *strings1[],
  size_t n2,
  const size_t *lengths2,
  const uint8_t *strings2[]);
typedef double(*SetSeqFuncUnicode)(size_t n1,
  const size_t *lengths1,
  const wchar_t *strings1[],
  size_t n2,
  const size_t *lengths2,
...
median_common(PyObject *args, const char *name, MedianFuncs foo) {
  ...
  if(stringtype == 0) {
    uint8_t *medstr = foo.s(n, sizes, strings, weights, &len);
    if(!medstr && len)
    ...
    wchar_t *medstr = foo.u(n, sizes, strings, weights, &len);
    if(!medstr && len)
      result = PyErr_NoMemory();
    else {
...
median_improve_common(PyObject *args, const char *name, MedianImproveFuncs foo) {
  ...
  Py_DECREF(strseq);
  if(stringtype == 0) {
    uint8_t *s = PyString_AS_STRING(arg1);
    size_t l = PyString_GET_SIZE(arg1);
    uint8_t *medstr = foo.s(l, s, n, sizes, strings, weights, &len);
    if(!medstr && len)
      result = PyErr_NoMemory();
    else {
      result = PyString_FromStringAndSize(medstr, len);
    ...
    wchar_t *s = PyUnicode_AS_UNICODE(arg1);
    size_t l = PyUnicode_GET_SIZE(arg1);
    wchar_t *medstr = foo.u(l, s, n, sizes, strings, weights, &len);
    if(!medstr && len)
      result = PyErr_NoMemory();
    else {
      result = PyUnicode_FromUnicode(medstr, len);
...
extract_stringlist(PyObject *list, const char *name,
  ...
  if(PyObject_TypeCheck(first, &PyString_Type)) {
    ...
    strings = (uint8_t**)malloc(n * sizeof(uint8_t*));
    if(!strings) {
  ...
  if(PyObject_TypeCheck(first, &PyUnicode_Type)) {
    ...
    strings = (wchar_t**)malloc(n * sizeof(wchar_t*));
    if(!strings) {
      PyErr_NoMemory();
      return -1;
    }
...
editops_py(PyObject *self, PyObject *args) {
  ...
  if(arg3) {
    ...
    if(len1 == std::string::npos || len2 == std::string::npos) {
      PyErr_Format(PyExc_ValueError,
 ...
 ****************************************************************************/
  ...
  **/
size_t lev_edit_distance(size_t len1, const uint8_t *string1,
  size_t len2, const uint8_t *string2,
  bool xcost) {
  ...
  /* initalize first row */
  size_t *row = (size_t*)malloc(len2 * sizeof(size_t)); /* we only need to keep one row of costs */
  if(!row)
    return std::string::npos;
  size_t *end = row + len2 - 1;
  for(size_t i = 0; i < len2 - (xcost ? 0 : half); i++)
    row[i] = i;
  ...
  if(xcost) {
    for(size_t i = 1; i < len1; i++) {
      size_t *p = row + 1;
      const uint8_t char1 = string1[i - 1];
      const uint8_t *char2p = string2;
      size_t D = i;
      size_t x = i;
      while(p <= end) {
        if(char1 == *(char2p++))
...
static double lev_edit_distance_sod(size_t len, const uint8_t *string,
  ...
  for(size_t i = 0; i < n; i++) {
    size_t d = lev_edit_distance(len, string, lengths[i], strings[i], xcost);
    if(d == std::string::npos)
      return -1.0;
    sum += weights[i] * d;
  }
 ...
 * Returns: The edit distance.
 **/
size_t lev_edit_distance(size_t len1, const wchar_t *string1,
...
static double lev_edit_distance_sod(size_t len, const wchar_t *string,
  size_t n, const size_t *lengths,
  const wchar_t *strings[],
  const double *weights,
  bool xcost) {
 ...
 ****************************************************************************/
  ...
  **/
size_t lev_hamming_distance(size_t len,
 ...
 * Returns: The Hamming distance.
 **/
size_t lev_u_hamming_distance(size_t len,
 ...
 **/
double lev_jaro_ratio(size_t len1, const uint8_t *string1,
  size_t len2, const uint8_t *string2) {
 ...
 * Returns: The Jaro metric of @string1 and @string2.
 **/
double lev_u_jaro_ratio(size_t len1, const wchar_t *string1,
  size_t len2, const wchar_t *string2) {
  size_t i, j, halflen, trans, match, to;
  size_t *idx;
  double md;
 ...
 **/
double lev_jaro_winkler_ratio(size_t len1, const uint8_t *string1,
  size_t len2, const uint8_t *string2,
  double pfweight) {
  double j;
  size_t p, m;
 ...
 * Returns: The Jaro-Winkler metric of @string1 and @string2.
 **/
double lev_u_jaro_winkler_ratio(size_t len1, const wchar_t *string1,
  size_t len2, const wchar_t *string2,
  double pfweight) {
  size_t p, m;

  double j = lev_u_jaro_ratio(len1, string1, len2, string2);
  m = len1 < len2 ? len1 : len2;
  for(p = 0; p < m; p++) {
    if(string1[p] != string2[p])
      break;
...
static uint8_t* make_symlist(size_t n, const size_t *lengths,
  const uint8_t *strings[], size_t *symlistlen) {
  /* indexed by ALL symbols, contains 1 for symbols
                         present in the strings, zero for others */
  short int symset[0x100] = {};

  if(!symset) {
    *symlistlen = std::string::npos;
    return NULL;
  }
  *symlistlen = 0;
  for(size_t i = 0; i < n; i++) {
    const uint8_t *stri = strings[i];
  ...
  size_t pos = 0;
  uint8_t *symlist = (uint8_t*)malloc((*symlistlen) * sizeof(uint8_t));
  if(!symlist) {
    *symlistlen = std::string::npos;
    return NULL;
  }
...
lev_greedy_median(size_t n, const size_t *lengths,
  ...
  uint8_t *symlist;  /* list of symbols present in the strings,
  ...
  uint8_t *median;  /* the resulting approximate median string */
   ...
   * a valid answer) */
  median = (uint8_t*)malloc(stoplen * sizeof(uint8_t));
  if(!median) {
    for(j = 0; j < n; j++)
      free(rows[j]);
    free(rows);
  ...
  /* return result */
  {
    uint8_t *result = (uint8_t*)malloc(bestlen * sizeof(uint8_t));
    if(!result) {
      free(median);
      return NULL;
    }
    memcpy(result, median, bestlen * sizeof(uint8_t));
    free(median);
    *medlength = bestlen;
    return result;
  }
 ...
 */
static double
finish_distance_computations(size_t len1, uint8_t *string1,
  size_t n, const size_t *lengths,
  const uint8_t **strings,
  ...
  for(j = 0; j < n; j++) {
    ...
    const uint8_t *stringi = strings[j];  /* current string */
 ...
 **/
uint8_t*
lev_median_improve(size_t len, const uint8_t *s,
  size_t n, const size_t *lengths,
  const uint8_t *strings[],
  const double *weights,
  size_t *medlength) {
  ...
  if(!symlist) {
    *medlength = 0;
    if(symlistlen != 0)
      ...
      return (uint8_t*)std::calloc(1, sizeof(uint8_t));
  }
  ...
  median = (uint8_t*)malloc((stoplen + 1) * sizeof(uint8_t));
  if(!median) {
    for(j = 0; j < n; j++)
      free(rows[j]);
    free(rows);
  ...
  medlen = len;
  memcpy(median, s, (medlen) * sizeof(uint8_t));
  ...
  /* return result */
  {
    uint8_t *result = (uint8_t*)malloc(medlen * sizeof(uint8_t));
    if(!result) {
      free(median);
      return NULL;
    }
    *medlength = medlen;
    memcpy(result, median, medlen * sizeof(uint8_t));
    median--;
    free(median);
    return result;
  }
...
make_usymlist(size_t n, const size_t *lengths,
  const wchar_t *strings[], size_t *symlistlen) {
  wchar_t *symlist;
  size_t i, j;
  HItem *symmap;
  ...
  *symlistlen = 0;
  ...
  {
    size_t pos = 0;
    symlist = (wchar_t*)malloc((*symlistlen) * sizeof(wchar_t));
    if(!symlist) {
      free_usymlist_hash(symmap);
      *symlistlen = std::string::npos;
      return NULL;
...
lev_u_greedy_median(size_t n, const size_t *lengths,
  ...
  size_t j;  /* usually iterates over characters */
  size_t len;  /* usually iterates over the approximate median string */
  wchar_t *symlist;  /* list of symbols present in the strings,
                              we iterate over it insead of set of all
                              existing symbols */
  ...
  size_t *row;  /* a scratch buffer for new Levenshtein matrix row computation,
                   shared among all strings */
  wchar_t *median;  /* the resulting approximate median string */
  double *mediandist;  /* the total distance of the best median string of
                          given length.  warning!  mediandist[0] is total
                          distance for empty string, while median[] itself
                          is normally zero-based */
  ...
  /* compute final cost of string of length 0 (empty string may be also
   * a valid answer) */
  median = (wchar_t*)malloc(stoplen * sizeof(wchar_t));
  if(!median) {
    for(j = 0; j < n; j++)
      free(rows[j]);
    free(rows);
  ...
  /* return result */
  {
    wchar_t *result = (wchar_t*)malloc(bestlen * sizeof(wchar_t));
    if(!result) {
      free(median);
      return NULL;
    }
    memcpy(result, median, bestlen * sizeof(wchar_t));
    free(median);
    *medlength = bestlen;
    return result;
  }
 ...
 */
static double
finish_udistance_computations(size_t len1, wchar_t *string1,
  size_t n, const size_t *lengths,
  const wchar_t **strings,
  const double *weights, size_t **rows,
  size_t *row) {
  ...
  for(j = 0; j < n; j++) {
    ...
    size_t leni = lengths[j];  /* current length */
    size_t len = len1;  /* temporary len1 for suffix stripping */
    const wchar_t *stringi = strings[j];  /* current string */
 ...
 **/
wchar_t*
lev_u_median_improve(size_t len, const wchar_t *s,
  size_t n, const size_t *lengths,
  const wchar_t *strings[],
  const double *weights,
  size_t *medlength) {
  ...
  /* initialize median to given string */
  median = (wchar_t*)malloc((stoplen + 1) * sizeof(wchar_t));
  if(!median) {
    for(j = 0; j < n; j++)
      free(rows[j]);
    free(rows);
  ...
  median++;  /* we need -1st element for insertions a pos 0 */
  medlen = len;
  memcpy(median, s, (medlen) * sizeof(wchar_t));
  ...
  /* return result */
  {
    wchar_t *result = (wchar_t*)malloc(medlen * sizeof(wchar_t));
    if(!result) {
      free(median);
      return NULL;
    }
    *medlength = medlen;
    memcpy(result, median, medlen * sizeof(wchar_t));
    median--;
    free(median);
    return result;
  }
...
static uint8_t* make_symlistset(size_t n, const size_t *lengths,
  const uint8_t *strings[], size_t *symlistlen,
  double *symset) {
  size_t i, j;
  uint8_t *symlist;
  ...
  *symlistlen = 0;
  ...
  {
    size_t pos = 0;
    symlist = (uint8_t*)malloc((*symlistlen) * sizeof(uint8_t));
    if(!symlist) {
      *symlistlen = std::string::npos;
      return NULL;
    }
...
static uint8_t* lev_quick_median(size_t n,
  const size_t *lengths,
  const uint8_t *strings[],
  const double *weights,
  size_t *medlength) {
  ...
  uint8_t *symlist;
  uint8_t *median;  /* the resulting string */
  double *symset;
  double ml, wl;
  ...
  if(wl == 0.0)
    return (uint8_t*)calloc(1, sizeof(uint8_t));
  ml = std::floor(ml / wl + 0.499999);
  *medlength = len = ml;
  if(!len)
    return (uint8_t*)calloc(1, sizeof(uint8_t));
  median = (uint8_t*)malloc(len * sizeof(uint8_t));
  if(!median)
    return NULL;
  ...
  for(j = 0; j < len; j++) {
    ...
    for(i = 0; i < n; i++) {
      ...
      size_t istart = std::floor(start);
      size_t iend = std::ceil(end);
...
static wchar_t* lev_u_quick_median(size_t n,
  const size_t *lengths,
  const wchar_t *strings[],
  const double *weights,
  size_t *medlength) {
  size_t symlistlen, len, i, j, k;
  wchar_t *symlist;
  wchar_t *median;  /* the resulting string */
  HQItem *symmap;
  double ml, wl;
  ...
  *medlength = len = ml;
  if(!len)
    return (wchar_t*)calloc(1, sizeof(wchar_t));
  median = (wchar_t*)malloc(len * sizeof(wchar_t));
  if(!median)
    return NULL;
  ...
  for(i = 0; i < n; i++) {
    ...
    while(j < i && dist < mindist) {
      ...
      if(distances[dindex] >= 0)
        ...
        d = lev_edit_distance(lengths[j], strings[j], leni, stri, false);
        if(d < 0) {
          free(distances);
          return std::string::npos;
        }
 ...
 **/
uint8_t* lev_set_median(size_t n, const size_t *lengths,
  const uint8_t *strings[],
  const double *weights,
  size_t *medlength) {
  ...
  *medlength = lengths[minidx];
  ...
  result = (uint8_t*)malloc(lengths[minidx] * sizeof(uint8_t));
  if(!result)
    return NULL;
  return (uint8_t*)memcpy(result, strings[minidx], lengths[minidx] * sizeof(uint8_t));
}
...
lev_u_set_median(size_t n, const size_t *lengths,
  ...
  *medlength = lengths[minidx];
  ...
  result = (wchar_t*)malloc(lengths[minidx] * sizeof(wchar_t));
  if(!result)
    return NULL;
  return (wchar_t*)memcpy(result, strings[minidx], lengths[minidx] * sizeof(wchar_t));
}
/* }}} */
...
lev_edit_seq_distance(size_t n1, const size_t *lengths1,
  ...
  while(n1 > 0 && n2 > 0
    ...
    && memcmp(strings1[n1 - 1], strings2[n2 - 1],
      lengths1[n1 - 1] * sizeof(uint8_t)) == 0) {
    n1--;
    n2--;
  }
  ...
  for(i = 1; i < n1; i++) {
    double *p = row + 1;
    const uint8_t *str1 = strings1[i - 1];
    const size_t len1 = lengths1[i - 1];
    const uint8_t **str2p = strings2;
    ...
    while(p <= end) {
      ...
      if(l == 0)
        ...
        size_t d = lev_edit_distance(len1, str1, *(len2p++), *(str2p++), true);
        if(d == std::string::npos) {
          free(row);
          return -1.0;
        }
...
lev_u_edit_seq_distance(size_t n1, const size_t *lengths1,
  ...
  while(n1 > 0 && n2 > 0
    && lengths1[n1 - 1] == lengths2[n2 - 1]
    && memcmp(strings1[n1 - 1], strings2[n2 - 1],
      lengths1[n1 - 1] * sizeof(wchar_t)) == 0) {
    n1--;
    n2--;
  }
  ...
  for(i = 1; i < n1; i++) {
    double *p = row + 1;
    const wchar_t *str1 = strings1[i - 1];
    const size_t len1 = lengths1[i - 1];
    const wchar_t **str2p = strings2;
    const size_t *len2p = lengths2;
    double D = i - 1.0;
    double x = i;
    while(p <= end) {
...
lev_set_distance(size_t n1, const size_t *lengths1,
  ...
  for(i = 0; i < n2; i++) {
    ...
    const uint8_t *str2 = strings2[i];
    const size_t *len1p = lengths1;
    const uint8_t **str1p = strings1;
    for(j = 0; j < n1; j++) {
      size_t l = len2 + *len1p;
      if(l == 0)
        *(r++) = 0.0;
      else {
        size_t d = lev_edit_distance(len2, str2, *(len1p++), *(str1p)++, true);
        if(d == std::string::npos) {
          free(r);
          return -1.0;
        }
...
lev_u_set_distance(size_t n1, const size_t *lengths1,
  ...
  for(i = 0; i < n2; i++) {
    size_t len2 = lengths2[i];
    const wchar_t *str2 = strings2[i];
    const size_t *len1p = lengths1;
    const wchar_t **str1p = strings1;
    for(j = 0; j < n1; j++) {
      size_t l = len2 + *len1p;
      if(l == 0)
        *(r++) = 0.0;
 ...
 **/
uint8_t*
lev_editops_apply(size_t len1, const uint8_t *string1,
  size_t len2, const uint8_t *string2,
  size_t n, const LevEditOp *ops,
  size_t *len) {
  uint8_t *dst, *dpos;  /* destination string */
  const uint8_t *spos;  /* source string position */
  size_t i, j;
  ...
  dpos = dst = (uint8_t*)malloc((n + len1) * sizeof(uint8_t));
  if(!dst) {
    *len = std::string::npos;
    return NULL;
  }
  ...
  for(i = n; i; i--, ops++) {
    ...
    if(j) {
      memcpy(dpos, spos, j * sizeof(uint8_t));
      spos += j;
      dpos += j;
    }
    switch(ops->type) {
  ...
  *len = dpos - dst;
  ...
  return (uint8_t*)realloc(dst, *len * sizeof(uint8_t));
}
 ...
 **/
wchar_t*
lev_u_editops_apply(size_t len1, const wchar_t *string1,
  size_t len2,
  const wchar_t *string2,
  size_t n, const LevEditOp *ops,
  size_t *len) {
  wchar_t *dst, *dpos;  /* destination string */
  const wchar_t *spos;  /* source string position */
  size_t i, j;

  /* this looks too complex for such a simple task, but note ops is not
   * a complete edit sequence, we have to be able to apply anything anywhere */
  dpos = dst = (wchar_t*)malloc((n + len1) * sizeof(wchar_t));
  if(!dst) {
    *len = std::string::npos;
    return NULL;
  }
  ...
  for(i = n; i; i--, ops++) {
    ...
    j = ops->spos - (spos - string1) + (ops->type == LEV_EDIT_KEEP);
    if(j) {
      memcpy(dpos, spos, j * sizeof(wchar_t));
      spos += j;
      dpos += j;
    }
    switch(ops->type) {
  ...
  *len = dpos - dst;
  /* possible realloc failure is detected with *len != 0 */
  return (wchar_t*)realloc(dst, *len * sizeof(wchar_t));
}
 ...
 **/
static LevEditOp* editops_from_cost_matrix(size_t len1, const uint8_t *string1, size_t off1,
  size_t len2, const uint8_t *string2, size_t off2,
 ...
 **/
LevEditOp* lev_editops_find(size_t len1, const uint8_t *string1,
  size_t len2, const uint8_t *string2,
  size_t *n) {
  size_t len1o, len2o;
  size_t i;
 ...
 *          elementary edit operations, it length is stored in @n.
 **/
static LevEditOp* ueditops_from_cost_matrix(size_t len1, const wchar_t *string1, size_t o1,
  size_t len2, const wchar_t *string2, size_t o2,
  size_t *matrix, size_t *n) {
  size_t *p;
  size_t i, j, pos;
  LevEditOp *ops;
 ...
 *          It is normalized, i.e., keep operations are not included.
 **/
LevEditOp* lev_editops_find(size_t len1, const wchar_t *string1,
  size_t len2, const wchar_t *string2,
  size_t *n) {
  size_t len1o, len2o;
  size_t i;
 ...
 *          size is stored in @n.
 **/
LevEditOp* lev_opcodes_to_editops(size_t nb, const LevOpCode *bops,
...
lev_editops_to_opcodes(size_t n, const LevEditOp *ops, size_t *nb,
  ...
  const LevEditOp *o;
  LevOpCode *bops, *b;
  uint8_t/*LevEditType*/ type = LEV_EDIT_KEEP;;
 ...
 **/
uint8_t*
lev_opcodes_apply(size_t len1, const uint8_t *string1,
  size_t len2, const uint8_t *string2,
  size_t nb, const LevOpCode *bops,
  size_t *len) {
  uint8_t *dst, *dpos;  /* destination string */
  ...
  /* this looks too complex for such a simple task, but note ops is not
   * a complete edit sequence, we have to be able to apply anything anywhere */
  dpos = dst = (uint8_t*)malloc((len1 + len2) * sizeof(uint8_t));
  if(!dst) {
    *len = std::string::npos;
    return NULL;
  }
    ...
    switch(bops->type) {
      ...
      case LEV_EDIT_REPLACE:
        memcpy(dpos, string2 + bops->dbeg,
          (bops->dend - bops->dbeg) * sizeof(uint8_t));
        break;

      case LEV_EDIT_KEEP:
        memcpy(dpos, string1 + bops->sbeg,
          (bops->send - bops->sbeg) * sizeof(uint8_t));
        break;
 ...
 **/
wchar_t*
lev_u_opcodes_apply(size_t len1, const wchar_t *string1,
  size_t len2, const wchar_t *string2,
  size_t nb, const LevOpCode *bops,
  size_t *len) {
  wchar_t *dst, *dpos;  /* destination string */
  ...
  /* this looks too complex for such a simple task, but note ops is not
   * a complete edit sequence, we have to be able to apply anything anywhere */
  dpos = dst = (wchar_t*)malloc((len1 + len2) * sizeof(wchar_t));
  if(!dst) {
    *len = std::string::npos;
    return NULL;
  }
  ...
  for(i = nb; i; i--, bops++) {
    switch(bops->type) {
      ...
      case LEV_EDIT_REPLACE:
        memcpy(dpos, string2 + bops->dbeg,
          (bops->dend - bops->dbeg) * sizeof(wchar_t));
        break;

      case LEV_EDIT_KEEP:
        memcpy(dpos, string1 + bops->sbeg,
          (bops->send - bops->sbeg) * sizeof(wchar_t));
        break;
 ...
 *          stored in @nmblocks.
 **/
LevMatchingBlock* lev_opcodes_matching_blocks(size_t len1,
  size_t len2,
  size_t nb,
...
LevMatchingBlock* lev_opcodes_matching_blocks_2(size_t len1,
  size_t len2,
  size_t nb,
  const LevOpCode *bops,
  size_t *nmblocks) {
